...
betaFunc (BetaParams alpha beta) = ((gammaFunc alpha 10000) * 
                                   (gammaFunc beta 10000)) / gammaFunc (alpha + beta) 10000
...
incompleteBeta x (BetaParams alpha beta) =
   x**alpha * sum [((pocahammer (1-beta) (fromIntegral n)) / 
                   ((fromIntegral (factorial n)) * 
                   (alpha + (fromIntegral n)))) * x^n | n <- [0..20]] 
